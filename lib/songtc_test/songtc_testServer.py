...
def get_config():
    ...
    for nameval in config.items(get_service_name() or 'songtc_test'):
        retconfig[nameval[0]] = nameval[1]
    return retconfig

config = get_config()

from songtc_test.songtc_testImpl import songtc_test
impl_songtc_test = songtc_test(config)
...
class Application(object):
    ...
    def __init__(self):
        submod = get_service_name() or 'songtc_test'
        ...
        self.rpc_service.add(impl_songtc_test.filter_contigs,
                             name='songtc_test.filter_contigs',
                             types=[dict])
        self.method_authentication['songtc_test.filter_contigs'] = 'required'
        self.rpc_service.add(impl_songtc_test.status,
                             name='songtc_test.status',
                             types=[dict])
                    ...
                    if auth_req != "none":
                        if token is None and auth_req == 'required':
                            ...
                            err.data = "Authentication required for " + \
                                "songtc_test but no authentication header was passed"
                            raise err
