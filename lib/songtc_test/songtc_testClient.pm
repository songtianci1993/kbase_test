package songtc_test::songtc_testClient;
...
songtc_test::songtc_testClient

=head1 DESCRIPTION


A KBase module: songtc_test
...
sub new
{
    ...
    my $self = {
	client => songtc_test::songtc_testClient::RpcClient->new,
	url => $url,
	headers => [],
    };
...
<pre>
$params is a songtc_test.FilterContigsParams
$return is a songtc_test.FilterContigsResults
FilterContigsParams is a reference to a hash where the following keys are defined:
	workspace has a value which is a songtc_test.workspace_name
	contigset_id has a value which is a songtc_test.contigset_id
...
FilterContigsResults is a reference to a hash where the following keys are defined:
	...
	new_contigset_ref has a value which is a songtc_test.ws_contigset_id
...
# Authentication: required
	...
	method => "songtc_test.filter_contigs",
	params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
...
sub _validate_version {
    ...
    if ($sMinor > $cMinor) {
        warn "New client version available for songtc_test::songtc_testClient\n";
    }
    if ($sMajor == 0) {
        warn "songtc_test::songtc_testClient version is $svr_version. API subject to change.\n";
    }
}
...
package songtc_test::songtc_testClient::RpcClient;
